...
ifeq ($(findstring $(MAKECMDGOALS), n64), n64)
	N64_CHARACTER_PIXEL_WIDTH = 8
	N64_CHARACTER_PIXEL_HEIGHT = 8
	N64_RESOLUTION_WIDTH = 640
	N64_RESOLUTION_HEIGHT = 480
	DIMENSIONS_DEFS = -DN64_CHARACTER_PIXEL_WIDTH=$(N64_CHARACTER_PIXEL_WIDTH)
	DIMENSIONS_DEFS += -DN64_CHARACTER_PIXEL_HEIGHT=$(N64_CHARACTER_PIXEL_HEIGHT)
	DIMENSIONS_DEFS += -DN64_RESOLUTION_WIDTH=$(N64_RESOLUTION_WIDTH)
	DIMENSIONS_DEFS += -DN64_RESOLUTION_HEIGHT=$(N64_RESOLUTION_HEIGHT)
	DIMENSIONS_DEFS += -DBASE_BORDER_BOX_WIDTH=$$(( $(N64_RESOLUTION_WIDTH) / $(N64_CHARACTER_PIXEL_WIDTH) - 1 ))
	DIMENSIONS_DEFS += -DBASE_BORDER_BOX_HEIGHT=$$(( $(N64_RESOLUTION_HEIGHT) / $(N64_CHARACTER_PIXEL_HEIGHT) - 1 ))
	DIMENSIONS_DEFS += -DN64_ANALOG_X_DEADZONE_PERCENTAGE=20
	DIMENSIONS_DEFS += -DN64_ANALOG_Y_DEADZONE_PERCENTAGE=20
	DIMENSIONS_DEFS += -DN64_VIEWPORT_MAX_MOVEMENT_PER_SECOND=2
	BYTE_ORDER = -DBYTE_ORDER=BIG_ENDIAN
else
	DIMENSIONS_DEFS = -DBASE_BORDER_BOX_WIDTH=100
	DIMENSIONS_DEFS += -DBASE_BORDER_BOX_HEIGHT=35
	DIMENSIONS_DEFS += -DBASE_BORDER_BOX_X_POSITION=1
	DIMENSIONS_DEFS += -DBASE_BORDER_BOX_Y_POSITION=1
	BYTE_ORDER = -DBYTE_ORDER=LITTLE_ENDIAN
endif

INCLUDE_ALL_MONERO_MNEMONIC_LANGUAGES = -DINCLUDE_FRENCH_MONERO_MNEMONIC=1 -DINCLUDE_ITALIAN_MONERO_MNEMONIC=1 -DINCLUDE_DUTCH_MONERO_MNEMONIC=1 -DINCLUDE_PORTUGUESE_MONERO_MNEMONIC=1 -DINCLUDE_GERMAN_MONERO_MNEMONIC=1 -DINCLUDE_SPANISH_MONERO_MNEMONIC=1 -DINCLUDE_RUSSIAN_MONERO_MNEMONIC=1 -DINCLUDE_JAPANESE_MONERO_MNEMONIC=1 -DINCLUDE_CHINESE_MONERO_MNEMONIC=1 -DINCLUDE_ESPERANTO_MONERO_MNEMONIC=1 -DINCLUDE_LOJBAN_MONERO_MNEMONIC=1
INCLUDED_MONERO_MNEMONIC_LANGUAGES_IN_ALL_BUILDS =
CXXFLAGS = $(INCLUDE) -DUSE_MONERO=1 -DUSE_KECCAK=1 $(COLOR_DEFS) $(DIMENSIONS_DEFS) $(INCLUDED_MONERO_MNEMONIC_LANGUAGES_IN_ALL_BUILDS) $(DEFAULT_PATH_OVERRIDES)
CCFLAGS = $(INCLUDE) -DUSE_MONERO=1 -DUSE_KECCAK=1 $(BYTE_ORDER) $(INCLUDED_MONERO_MNEMONIC_LANGUAGES_IN_ALL_BUILDS)

CLI_MAKE_TARGET = $(findstring $(MAKECMDGOALS), cli)
ifeq ($(CLI_MAKE_TARGET), cli)
	CXXFLAGS += -std=c++11 -pthread -DUTF8_COMPAT_BUILD=1 $(INCLUDE_ALL_MONERO_MNEMONIC_LANGUAGES)
	CCFLAGS += $(INCLUDE_ALL_MONERO_MNEMONIC_LANGUAGES)
endif
ifeq ($(findstring $(MAKECMDGOALS), tests), tests)
	CXXFLAGS += $(INCLUDE_ALL_MONERO_MNEMONIC_LANGUAGES)
	CCFLAGS += $(INCLUDE_ALL_MONERO_MNEMONIC_LANGUAGES)
endif
...
ifeq ($(N64_MAKE_TARGET), n64)
	...
	CXXFLAGS += -DUTF8_COMPAT_BUILD=1 -DN64_RESEED_EVERY_X_V_BLANKS=15000
endif
